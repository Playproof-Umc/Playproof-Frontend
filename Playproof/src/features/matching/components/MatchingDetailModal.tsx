...
export const MatchingDetailModal = () => {
  ...
  // 안전성 체크
  if (!state) {
    console.error('useMatchingDetailLogic returned undefined state');
    return null;
  }
  
  const { shouldRender, selectedPost, isMenuOpen } = state;
  const userId = useAuthStore(state => state.userId);
  const { getCommentCount } = useMatchingDetail();

  if (!shouldRender || !selectedPost) return null;
  
  // selectedPost 유효성 검증
  if (!selectedPost.hostUser) {
    console.error('selectedPost.hostUser is undefined', selectedPost);
    return null;
  }
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl w-full max-w-[1000px] h-[85vh] max-h-[700px] shadow-2xl flex overflow-hidden relative animate-in zoom-in-95 duration-200">
        ...
        <MatchingPostInfo 
          post={selectedPost} 
          commentCount={getCommentCount(selectedPost)}
